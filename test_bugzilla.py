...
def bz_read_page(bug_id, view_type='bug_view', print_html=True):
    ...
    # TODO: we need to do a headless browser launch here.
    # times are not the same for page print
    if print_html:
        # print(resp.text)
...
def bz_quicksearch(search_string):
    bz = BugzillaRESTClient()
    return bz.bug_quicksearch(search_string)


@pytest.mark.skip(reason="TBD")
def test_search_quicksearch(benchmark, rounds):
    search_string = 'lorem'
...
@pytest.mark.skip(reason="TBD")
def test_read_bug_with_3_comments(benchmark, rounds):
    bug_id = bug_with_comments(3, 'BUG_WITH_3_COMMENTS')
    benchmark.pedantic(bz_read_page, rounds=rounds, kwargs={'bug_id': bug_id})


@pytest.mark.skip(reason="TBD")
def test_read_bug_with_100_comments(benchmark, rounds):
    bug_id = bug_with_comments(100, 'BUG_WITH_100_COMMENTS')
    benchmark.pedantic(bz_read_page, rounds=rounds, kwargs={'bug_id': bug_id})


# @pytest.mark.skip(reason="TBD")
def test_read_bug_with_100_comments_plus_links(benchmark, rounds):
    bug_id = bug_with_comments(100, 'BUG_WITH_100_COMMENTS_PLUS_LINKS',
                               comment_char_size=5000, add_links=True,
...
def test_read_bug_with_200_comments(benchmark, rounds):
    bug_id = bug_with_comments(200, 'BUG_WITH_200_COMMENTS')
    benchmark.pedantic(bz_read_page, rounds=rounds, kwargs={'bug_id': bug_id})


@pytest.mark.skip(reason="TBD")
def test_read_bug_with_300_comments(benchmark, rounds):
    bug_id = bug_with_comments(300, 'BUG_WITH_300_COMMENTS')
    benchmark.pedantic(bz_read_page, rounds=rounds, kwargs={'bug_id': bug_id})
